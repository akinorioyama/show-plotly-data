javascript:(function() {
    ...
    const prNumberInput = prompt("Enter text (or leave blank for all):", "");
    let targetText = null;
    ...
    if (prNumberInput.trim() !== "") {
		targetText = prNumberInput;
		filterIsActive = true;
    } else {
        alert("No text filter applied.");
    }
    ...
    try {
        ...
        for (let i = 0; i < traces.length; i += 2) {
            ...
            for (let j = 0; j < numPoints; j++) {
                const customtext = trace.text && trace.text[j] ? trace.text[j] : {};
                const customdata = trace.customdata && trace.customdata[j] ? trace.customdata[j] : {};				
                const textValue = String(customtext || "");
                if (!filterIsActive || (textValue.indexOf(targetText))!==-1) {
